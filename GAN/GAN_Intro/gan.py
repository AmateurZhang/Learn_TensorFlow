...
def linear(input, output_dim, scope=None, stddev=1.0):
    with tf.variable_scope(scope or 'linear'): # g0,d1/w or b
        w = tf.get_variable(
            'w',
            [input.get_shape()[1], output_dim],
            initializer=tf.random_normal_initializer(stddev=stddev)
        ) # initialize the w with [cols,output_dim] shape, normal N(0,1) values.
        b = tf.get_variable(
            'b',
            [output_dim],
...
def generator(input, h_dim):
    ...
    # h0 = log(exp(g0/w*input+g0/b)+1) h0.dim=h_dim
    h1 = linear(h0, 1, 'g1')
    # h1  = g1/w * h0 +g1/b, h1.dim=1
    return h1
